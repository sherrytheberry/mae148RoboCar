...
def check_voltage_and_run_manage():
    # Define the serial port (replace '/dev/ttyUSB0' with the actual port)
    ...
    ser.write(b'Hello from Jetson Nano\n')
    print("Sent command to Arduino")
    
    # Flag to ensure manage.py drive is only run once
    manage_run = False

    # Specify the directory to save the map and CSV file
    save_dir = '/home/jetson/projects/mycar'
    ...
    while True:
        ...
        if voltage_str:
            print("Received:", voltage_str)
            try:
                voltage = float(voltage_str)
            except ValueError:
                print("Invalid voltage value received.")
                continue
        else:
        ...
        if 14.7 <= voltage <= 14.8:
         
            if not manage_run:
                # Close serial port
                ser.close()
                
                #print("Low voltage detected. Running manage.py drive...")
                generate_map()
                #drive(cfg, use_joystick=args['--js'], camera_type=args['--camera'])
                manage_run = True  # Ensure manage.py drive is only run once
	
        time.sleep(1)

# Google Maps API
def get_path_coordinates(api_key, origin_lat, origin_lng, dest_lat, dest_lng, num_intermediate_points):
    url = f"https://maps.googleapis.com/maps/api/directions/json?origin={origin_lat},{origin_lng}&destination={dest_lat},{dest_lng}&mode=walking&key={api_key}"
    ...
    if response.status_code == 200:
        ...
        if data['status'] == 'OK':
            ...
            for step in steps:
                ...
                # Calculate intermediate points between start and end of each step
                intermediate_lats = [start_lat + i * (end_lat - start_lat) / (num_intermediate_points + 1) for i in range(1, num_intermediate_points + 1)]
                intermediate_lngs = [start_lng + i * (end_lng - start_lng) / (num_intermediate_points + 1) for i in range(1, num_intermediate_points + 1)]
                
                # Add intermediate coordinates to the path
                for lat, lng in zip(intermediate_lats, intermediate_lngs):
                    path_coordinates.append((lat, lng))
                
            # Add the destination coordinates as well
            path_coordinates.append((dest_lat, dest_lng))
...
def generate_map():  
    ...
    api_key = 'xxx' #replace with your own API key from google map

    # Assuming you have already set and converted GPS data to latitude_test and longitude_test
    latest_longitude = V.mem['gps/longitude']
    latest_latitude = V.mem['gps/latitude']
    if latest_longitude is not None and latest_latitude is not None:
        # Convert the latest_longitude and latest_latitude using utm.to_latlon
        latitude_test, longitude_test = utm.to_latlon(latest_longitude, latest_latitude, 11, 'S')
        print("Latest GPS coordinates saved successfully.")
        print("Lat:", latitude_test)
        print("Long:", longitude_test)
        
        origin_lat = latitude_test
        origin_lng = longitude_test    
    else:
        print("GPS data not available.")
        return
    
    dest_lat = 32.88125413296228   # Replace with your destination latitude
    dest_lng = -117.23518111925324  # Replace with your destination longitude 

    num_intermediate_points = 300  # Increase this number to output more coordinates
    
    path_coordinates = get_path_coordinates(api_key, origin_lat, origin_lng, dest_lat, dest_lng, num_intermediate_points)
    if path_coordinates:
        print("Path Coordinates (Lat, Lng):")
        for coord in path_coordinates:
            print(coord)
        
        # Convert path coordinates to UTM
        path_utm_coordinates = [utm.from_latlon(lat, lng)[:2] for lat, lng in path_coordinates]
        
        # Calculate relative UTM coordinates
        first_utm_coord = path_utm_coordinates[0]
        relative_utm_coordinates = [(easting - first_utm_coord[0], northing - first_utm_coord[1]) for easting, northing in path_utm_coordinates]
        
        print("\nPath Coordinates (Relative UTM):")
        for relative_utm_coord in relative_utm_coordinates:
            print(relative_utm_coord)
        
        # Create map centered around the origin coordinates
        map_obj = folium.Map(location=[origin_lat, origin_lng], zoom_start=13)
        
        # Add markers for origin and destination
        folium.Marker(location=[origin_lat, origin_lng], popup='Origin').add_to(map_obj)
        folium.Marker(location=[dest_lat, dest_lng], popup='Destination').add_to(map_obj)
        
        # Add polyline for the path
        folium.PolyLine(locations=path_coordinates, color='blue').add_to(map_obj)
        ...
        map_obj.save(os.path.join(save_dir, 'path_map.html'))
        print("Map generated! Check the specified directory for 'path_map.html'.")
        
        # Save coordinates to donkey.csv
        csv_path = os.path.join(save_dir, 'donkey.csv')
        if os.path.exists(csv_path):
            os.remove(csv_path)
            print("Existing 'donkey.csv' file deleted.")  
        
        with open(csv_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            for easting, northing in relative_utm_coordinates:
                writer.writerow([easting, northing, 0])
        
        print(f"CSV file generated! Check the specified directory for 'donkey.csv'.")
        
        while True:
            threshold_lat_lon = 0.00005
            recent_longitude = V.mem['gps/longitude']
            recent_latitude = V.mem['gps/latitude']
            print("Lat:", recent_longitude)
            print("Long:", recent_latitude)
            
            if latest_longitude is not None and latest_latitude is not None:
                # Convert the latest_longitude and latest_latitude using utm.to_latlon
                latitude_recent, longitude_recent = utm.to_latlon(recent_longitude, recent_latitude, 11, 'S')
                print("Latest GPS coordinates saved successfully.")
                print("Lat:", latitude_recent)
                print("Long:", longitude_recent)
            
                # Check if the latest GPS coordinates match the destination coordinates
                if (abs(latitude_recent - dest_lat) < threshold_lat_lon and abs(longitude_recent - dest_lng) < threshold_lat_lon):
                    
                    os._exit(0)
                    # V.stop()
                    # print("Reached destination coordinates.")
                    #V.start(rate_hz=cfg.DRIVE_LOOP_HZ,max_loop_count=1000)

                    #cfg.PID_THROTTLE = 0.0
                    ...
                    #cfg.AI_THROTTLE_MULT=0
                    #cfg.VESC_MAX_SPEED_PERCENT=0
                    print("I'm stopping")
                    break  # Exit the loop

            # Sleep to avoid busy waiting (adjust the sleep time as necessary)
            time.sleep(1)        
...
def add_gps(V, cfg):
    if cfg.HAVE_GPS:
        ...
        V.add(gps_latest_position, inputs=['gps/positions'], outputs=['gps/timestamp', 'gps/longitude', 'gps/latitude'])

        # rename gps utm position to pose values
        V.add(Pipe(), inputs=['gps/longitude', 'gps/latitude'], outputs=['pos/x', 'pos/y'])

        return nmea_player

def parse_nmea_sentences(nmea_sentences):
    for ts, nmea in nmea_sentences:
...
if __name__ == '__main__':
    ...
    thread1 = threading.Thread(target=drive, args=(cfg,)) 
    thread2 = threading.Thread(target=check_voltage_and_run_manage)
    thread1.start()
    thread2.start()
